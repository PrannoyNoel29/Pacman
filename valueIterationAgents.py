...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        "*** YOUR CODE HERE ***"
        quality = 0
        # get next state and probabilities
        stateprob = self.mdp.getTransitionStatesAndProbs(state,action)
        for future, chance in stateprob:
            reward = self.mdp.getReward(state, action, future)
            nextQval = self.getValue(future)
            quality += chance * (reward + self.discount * nextQval)
        # endfor
        return quality 
